...
const NARRATIONS = [
    { name: "0_introduction", text: "プロジェクトという名のクエストを攻略せよ。ジェミニ 2.5 フラッシュを搭載した次世代プロジェクト管理ツール、プロジェクト・クエストの全貌を紹介します。" },
    ...
    { name: "5_report_generation", text: "そしてプロジェクト管理の真骨頂、報告書生成機能です。TeamsやSlackなどのチャットログに加え、ダブリュービーエス、レッドマイン、そしてプロジェクト管理とは切り離せない、おかねの情報。それらを読ませることで、現場の生々しい実態を多角的に分析します。単なる資料作成ではなく、ジェミニ 2.5 フラッシュが本質を見抜いた『ガチの報告』を爆速で生成。表示されたプレビューはエディタで自由に書き換え可能で、人間が最後の魂を吹き込みます。この共創のプロセスが、報告という孤独な作業を、戦略的な意思決定へと変貌させます。" },
    { name: "6_mode_switch", text: "プロジェクトの空気に合わせてデザインを切り替え。情緒あるRPGモードと、端正なビジネスモード。どちらもデータの透明性は変わりません。もちろん、画面の内容でピーディーエフをダウンロードできます。" },
    { name: "7_council_meeting", text: "ここがProject Questの心臓部、エーアイ評議会です。ご覧ください、この流れるようなストリーミング。異なる専門性を持つ8人のエーアイエージェントたちが、リアルタイムで議論を繰り広げています。彼らはただ同調し合うのではありません。あるエーアイは納期を心配し、別のエーアイは技術的負債に警鐘を鳴らす。あえて設計された『対立構造』によって、議論の解像度は飛躍的に高まり、人間の盲点を突く鋭い意見が次々と飛び出します。あなたがすべきことは、この熱い議論を俯瞰し、納得できる案を見極めること。そして、特定の案を『採用』すれば、評議会はさらに次のフェーズへ。選ばれたコンセプトをどう具体化し、どうリスクを回避するか。議論はさらに深掘りされ、明日から現場が動ける『収束』のプロセスへと向かいます。このスピード、この深さ。もはやPMは一人で悩む必要はありません。エーアイたちの知性を借り、あなたは最高のアウトプットを出すための『意思決定』に、そのすべてのエネルギーを注ぎ込んでください。チームの総意をエーアイが作り上げ、あなたが最後の一おしをする。これが、次世代のプロジェクト攻略スタイルです。" },
