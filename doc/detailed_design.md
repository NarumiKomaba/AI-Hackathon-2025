...
*   **Summary**: WBS/Issue/Chat/Budgetデータを分析し、スライド構成用JSONを生成。
*   **AIプロンプト戦略**:
...
#### 4. PDF Generation API (Replacement of /api/report-pptx)
*   **Endpoint**: `POST /api/report-pdf`
*   **Summary**: Playwrightを使用してプレビュー画面を描画し、PDFとして出力する。
*   **Request Body**: `{ projectId: string }`
...
sequenceDiagram
    participant User
    participant Page as Report Preview Page
    participant API_Save as /api/report-save
    participant API_PDF as /api/report-pdf
    participant DB as Firestore
    participant PW as Playwright (Server)

    User->>Page: Click "保存してPDF出力"
    Page->>API_Save: POST /api/report-save (slides)
    API_Save->>DB: Update progress_report
    API_Save-->>Page: OK

    Page->>API_PDF: POST /api/report-pdf (projectId)
    API_PDF->>PW: Launch Browser & Navigate to Preview
    PW->>Page: Render Slides
    API_PDF->>PW: Export PDF (1280x720)
    API_PDF-->>Page: Binary Blob (PDF)

    Page->>User: Download .pdf File
```
...
| **Report API** | 各種データを統合しPDFファイルを生成 | Backend | `app/api/report-pdf/route.ts` |
